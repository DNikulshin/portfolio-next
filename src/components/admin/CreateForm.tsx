"use client"
import { createNewWork } from '@/hooks/useWork';
...
export const CreateForm = ({ userId }: { userId?: string }) => {
    ...
    const createWork = createNewWork()
    ...
    const handleSubmit: FormEventHandler<HTMLFormElement> = async (e) => {
        ...
        if (!formValue.title || !formValue.linkPath || !formValue.image || !userId) return;
        formData.append('title', formValue.title.trim());
        formData.append('linkPath', formValue.linkPath.trim())
        formData.append('image', formValue.image)

        createWork.mutate({ ...formValue, userId }, {
            onSuccess: () => {
                setFormValue({ title: '', linkPath: '', image: null as unknown as File | Blob, userId })
    ...
    return (
        <form className='flex flex-col gap-4 mx-auto container' onSubmit={handleSubmit}>
            <input
                type="text"
                name='title'
